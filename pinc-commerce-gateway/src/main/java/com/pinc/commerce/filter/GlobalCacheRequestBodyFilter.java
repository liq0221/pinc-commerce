...
public class GlobalCacheRequestBodyFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 判断是否是登陆或注册请求
        boolean isLoginOrRegister = exchange.getRequest().getURI().getPath().contains(GatewayConstants.LOGIN_URI)
                || exchange.getRequest().getURI().getPath().contains(GatewayConstants.REGISTER_URI);
        if (Objects.isNull(exchange.getRequest().getHeaders().getContentType())
                || !isLoginOrRegister) {
